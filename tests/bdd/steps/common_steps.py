...
def behave_table_to_duckdb_relation(table: Any, con: duckdb.DuckDBPyConnection = None) -> duckdb.DuckDBPyRelation:
...
def step_assert_dataframe_matches_table(context):
    """Compare the result DataFrame against a Gherkin table using DuckDB relations."""
    ...
    # Select only the columns defined in the expected table
    expected_cols = [col.split(":")[0] for col in context.table.headings]
    actual_df = context.result.select(expected_cols)
    actual = duckdb.sql("SELECT * FROM actual_df")
    compare_duckdb_relations(expected, actual)
